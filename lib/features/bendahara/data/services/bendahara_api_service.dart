...
class BendaharaApiService {
  BendaharaApiService({http.Client? client}) : _client = client ?? http.Client();

  final http.Client _client;

  Future<BendaharaOverview> fetchOverview(String token, {String? santriId}) async {
    final payload = await _getJson('mobile/santri/bendahara', token, santriId: santriId);
    if (payload['success'] == true) {
      return BendaharaOverview.fromJson(payload);
    }

    throw ApiException('${payload['message'] ?? 'Gagal memuat data bendahara.'}');
  }
